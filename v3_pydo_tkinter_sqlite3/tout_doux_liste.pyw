...
def database() : 
    ...
    # Activation de la vérification des clés étrangères (pas activé de base)
    connection.execute("PRAGMA foreign_keys = ON;")
    
    # Ordre pour l'autoincrement avec sqlite3 = INTEGER PRIMARY KEY AUTOINCREMENT !!!! attention à la syntaxe
    mycursor.execute(f"CREATE TABLE IF NOT EXISTS [etat]([id_etat] INTEGER PRIMARY KEY AUTOINCREMENT,[nom_etat] VARCHAR(10) UNIQUE);")
    
    mycursor.execute(f"CREATE TABLE IF NOT EXISTS [tache]([id_tache] INTEGER PRIMARY KEY AUTOINCREMENT,[nom_tache] VARCHAR(50) NOT NULL,[date_de_creation] DATE NOT NULL,[date_objectif] DATE, [date_de_realisation] DATE,[id_etat] INTEGER NOT NULL, FOREIGN KEY(id_etat) REFERENCES etat(id_etat));")

    mycursor.execute(f"INSERT INTO etat (nom_etat) VALUES ('À faire'),('En cours'),('Terminée') ON CONFLICT (nom_etat) DO NOTHING;")
...
def logo():
    ...
    label_icone = Label(frame, image=img)
    label_icone.image = img
    label_icone.pack(side=tk.LEFT, padx=30)
...
def window_app():
    ...
    # Etat initial des fenêtres ajouter et modifier
    ...
    #liaison de la touche 'suppr' à la fonction de suppression de tâche
    display_list.bind("<Delete>", delete_task_window)
...
def treeview ():
    ...
    table.column("Tâche", width=360, anchor=CENTER, minwidth = 360 )
...
def to_do_list(table):
    ...
    for row in list :
        dictionary_entry = {
            'nom_tache': row[0],
            'date_de_creation': datetime.strptime(row[1], '%Y-%m-%d').date(),
            'date_objectif': datetime.strptime(row[2][:10], '%Y-%m-%d').date(), #prend seulement les 10 premiers caractères (la date sans l'heure)
            'date_de_realisation': row[3],
            'nom_etat': row[4],
        }
    ...
    for entry in dictionary_list:
        ...
        finish_date_formatted = entry['date_de_realisation'] if entry['date_de_realisation'] else ""
        ...
        # Application du tag de la couleur en fonction du statut de la tâche
...
def close_add_task_window():
    ...
    if add_task_wind:
        ...
        window_states["add_task"] = False
        window_states["edit_task"] = False

def close_edit_task_window():
...
# Limitation du nombre de caractère de la tâche
def add_task_validate_input(task_name):
    max_lenght = 50
    if len(task_name) > max_lenght:
        messagebox.showerror("Erreur", f"La longueur maximale autorisée est de 50 caractères.")
...
def add_task_window(event):
     ...
     # Vérification si la fenêtre est déjà ouverte
    if window_states["add_task"] == True or window_states["edit_task"] == True:
        # Si oui, ne rien faire
        return
    
    else:
        # si pas de fenêtre ouverte, en créer une
        global add_task_wind
        add_task_wind = new_window("Ajouter une tâche")

        # Mise à jour de l'état de la fenêtre
        window_states["add_task"] = True
        ...
        validation = name_task_frame.register(add_task_validate_input)
        global input_task
        input_task = Entry(name_task_frame, bd = 1, width = 80, font = (font, 10), validate="key", validatecommand=(validation, '%P'))
        input_task.pack(side=LEFT, padx=10, pady=15)
        input_task.focus_set()
        ...
        # Liaison de la fonction de fermeture de la fenêtre à l'événement de fermeture de la fenêtre
        add_task_wind.protocol("WM_DELETE_WINDOW", close_add_task_window)
...
def add_task (event):
    ...
    escaped_task = f"{escaped_task}"

    creation_date = date.today()
    set_date = input_target_date.get_date() if input_target_date.get() else None
    id_task = "1"
    
    # Vérif si nom de la tâche existe déjà
    mycursor.execute("SELECT * FROM tache WHERE nom_tache = ?", (escaped_task,))
    existing_task = mycursor.fetchone()
    
    if existing_task:
        messagebox.showwarning("Avertissement", f"La tâche '{escaped_task}' existe déjà.\nMerci de donner un autre nom à votre tâche.")
        add_task_wind.deiconify()
        input_task.focus_set()
        return
    else:
...
def edit_task_validate_input(task_name):
    max_lenght = 50
    if len(task_name) > max_lenght:
        messagebox.showerror("Erreur", f"La longueur maximale autorisée est de 50 caractères.")
...
def edit_task_window(event):
    ...
    if selected_edit_item:
        ...
        mycursor.execute(f"SELECT id_tache, id_etat, date_objectif FROM tache WHERE nom_tache = ?", (selected_edit_task_values[1],))
        global edit_query_result
        edit_query_result = mycursor.fetchone()
        
        if edit_query_result:
            global edit_id_task
            edit_id_task = edit_query_result[0]
            global edit_id_state
            edit_id_state = edit_query_result[1]
        
        
        # Vérification si la fenêtre est déjà ouverte
        if window_states["edit_task"] == True or window_states["add_task"] == True:
            # Si oui, ne rien faire
            return
            ...
            validation = frame_update.register(edit_task_validate_input)
            global new_input_task
            new_input_task = Entry(frame_update, bd = 1, width = 30, font = (font, 10), validate="key", validatecommand=(validation, '%P'))
            ...
            new_input_task.focus_set()
            #liaison de la touche "entrer" à la commande "validate"
            new_input_task.bind("<Return>", validate)
            ...
            # Liaison de la fonction de fermeture de la fenêtre à l'événement de fermeture de la fenêtre
            edit_task_wind.protocol("WM_DELETE_WINDOW", close_edit_task_window)
...
def validate(event):
    ...
    # Vérif si nom de la tâche existe déjà
    mycursor.execute("SELECT * FROM tache WHERE nom_tache = ?", (new_name,))
    existing_task = mycursor.fetchone()

    if existing_task and new_name != old_name:
        messagebox.showwarning("Avertissement", f"La tâche '{new_name}' existe déjà.\nMerci de donner un autre nom à votre tâche.")
        edit_task_wind.deiconify() 
        new_input_task.focus_set()
        return
    
    elif new_name == '':
        messagebox.showwarning("Avertissement", f"Merci de renseigner un nom de tâche.")
        edit_task_wind.deiconify() 
        new_input_task.focus_set()
        return
        ...
        if new_state != old_state or new_name != old_name or new_target_date != old_target_date:
            if new_state != old_state:
                mycursor.execute(f"UPDATE tache SET id_etat = '{new_state}' WHERE id_tache = '{edit_id_task}'")
                if new_state == 3 :
                ...
                close_edit_task_window()
            else :
                pass

            if new_name != old_name:
                new_name = new_name.replace("'", "''")
                mycursor.execute(f"UPDATE tache SET nom_tache = '{new_name}' WHERE id_tache = '{edit_id_task}'")
                close_edit_task_window()
            else :
                pass
            ...
            if new_target_date != old_target_date:
                if new_target_date < selected_edit_task_values[2]:
                    ...
                    mycursor.execute(f"UPDATE tache SET date_objectif = '{new_target_date}' WHERE id_tache = '{edit_id_task}'")
                    close_edit_task_window()
            update_table(None)
...
def start_task_window(event):
    ...
    if selected_items:
        for selected_item in selected_items:
            ...
            # Requête pour obtenir l'id_tache en fonction du nom de la tâche
            mycursor.execute(f"SELECT id_tache, id_etat FROM tache WHERE nom_tache = ?", (selected_task_name,))
            query_result = mycursor.fetchone()

            if query_result:
                id_etat = query_result[1]
                id_tache = query_result[0]

                if id_etat == 1:
                    confirmation = messagebox.askyesno("Confirmation", f"Êtes-vous sûr de vouloir démarrer la tâche '{selected_task_name}'?")

                    if confirmation:
                        mycursor.execute(f"UPDATE tache SET id_etat = '2' WHERE id_tache = '{id_tache}'")
        ...
        # mise à jour du tableau une fois les tâches traitées
        update_table(event)
    else :
        ...
        for selected_item in selected_items:
            ...
            if query_result:
                ...
                if id_etat == 1 or id_etat == 2:
                    ...
                    if confirmation:
                        mycursor.execute(f"UPDATE tache SET id_etat = '3' WHERE id_tache = '{id_tache}'")
                        completion_date = date.today()
